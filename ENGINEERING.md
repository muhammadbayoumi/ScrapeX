...
- **W4 ONE product version, and it moves when the behaviour does.** `scrapex/version.py` is the authority; `pyproject.toml` and `extension/manifest.json` are mirrors and a drift test fails when either disagrees. Every capability a person would notice the absence of is recorded in `version.CAPABILITIES` with the version it is guaranteed from, the commit that built it, and — when the panel executes it — the control it is reached by; a capability without its version is a `TypeError` at import, and `contracts/capability-baseline.json` fails the build when the capability set changes while `VERSION` stands still. `MINIMUM_EXTENSION_VERSION` is **derived** from the ledger, never typed, and only panel capabilities raise it: an engine-only change must not send the owner to reload a browser. The compatibility rule lives in two languages (`version.capability_problem` / `extension/version.js`) pinned to `contracts/version-vectors.json`, and every refusal names BOTH versions and which side is behind — a stale extension and a stale engine have opposite remedies. `CHANGELOG.md` is generated from the ledger, never hand-written. *(Issue 32 §1. Provenance: the version string stood at 0.1.0 for 130 commits, so the product could not answer "does this build have X" and the owner asked twice in two days for features that had already shipped.)*
