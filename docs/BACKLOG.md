...
[app.py:2251](../scrapex/webui/app.py#L2251). So the eleven are invisible in the chooser and
...
| `scrapex/webui/app.py:2793` — `_about` | `worker_is_alive(conn)`, single heartbeat | **the function the fix superseded** |
...
`app.py:2793`; the heartbeat's behaviour under a held write lock; and the 409 on
...
`threading.Lock` ([scrapex/webui/app.py:2923](../scrapex/webui/app.py#L2923)) refusing a
concurrent build with the house 409; `BUNDLE_KEEP = 2`
([scrapex/webui/app.py:2904](../scrapex/webui/app.py#L2904)) pruning **by stamp** so the
two files of one backup cannot be split; and an age-guarded sweep
([scrapex/webui/app.py:2960](../scrapex/webui/app.py#L2960)) for orphaned staging.
