...
### OP-44 · The merge's own refusal sends the loser of a race to a flag that does not exist
...
### OP-45 · `merge-warehouse --claim` is declared and never read
...
it, or read it. Fix it in the same change as `OP-44`, since both are the same
command's surface.

### OP-46 · `merge-warehouse` silently discards the other machine's ENTIRE price history
**Found 2026-08-22. THE MOST SERIOUS THING IN THIS FILE. Not fixed — the fix needs
`Q-23` answered, and the 15-line refusal that stops the harm does not.**
...
**HAS IT ALREADY HAPPENED? Not knowable from this side, and the reason is the
finding.** `merge` writes **no ledger row anywhere** — no merge table, no
...
This is the one genuine design choice inside the price-merge fix `OP-46` names, and
