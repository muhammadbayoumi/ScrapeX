...
| 14 | The extension is **ScrapeX**; the local engine is **ScrapeX-Engine**. | In the catalogue: `ScrapeX-Engine — installed, v1.2.3` beside `Scrapy — not installed`. **Amended 2026-08-05**: the bare word `Engine` was the role, not a name, and the catalogue is a list of products — a row headed `Engine` beside one headed `Scrapy` reads as a category beside a product. Decision 15 is unaffected: the package is still `scrapex/`. |
...
| 24 | **The panel opens on Welcome: one line and one button.** | Before sign-in nothing a profile page would print is knowable — no account, no last backup, no lease holder — and a column of em dashes is not information. What the page shows after sign-in is decided when sign-in exists (M1). |
| 25 | **The icon rail groups pages by who serves them.** Profile and Engines in one container; Source, Run, Data and Google Finance in the next. | The second group is dead on a device with no engine installed, which is every device for its first minute. The boundary is drawn rather than explained. Google Finance moved into it: it is served by the engine exactly as the crawl pages are. |
| 26 | **`class` is for styling and nothing else; a class that resolves to no rule fails a test.** JS hooks and test selectors use `data-*` or `id`. | Two buttons shipped styled by `btn` and `icon-btn`, which no stylesheet defines, while the vocabulary they needed — `ghost`, `icon-button`, `compact` — already existed. 994 class names across 18 stylesheets and no place to look. The UI track UI-0..UI-3 sits between M0 and M1 because M1, M2 and M3 are page-heavy and every page built before the kit exists gets rewritten after it. See [docs/UI-KIT.md](UI-KIT.md). |
| 27 | **Backup timing is entirely the owner's.** Four whole answers to "when" — after any crawl that changed something (default) · every N hours · only when pressed · off — plus a shortest-gap ceiling he sets, where zero means none. | He asked for it directly: «اعمل اعدادات بحيث يمكن للمتسخدم التحكم الكامل فى ذلك الامر». The numbers the choice is about: the bundle is 209 MB and **33 MB compressed in one second**, small enough that "after every change" is affordable and large enough that "every hour regardless" is not. Every refusal NAMES the setting that caused it, because a backup that did not happen is indistinguishable from one that failed. `off` refuses even a press — "off" that still uploads is not off. |
| 28 | **The Console both creates a Sheet and accepts one.** Two buttons: *Create a new sheet* (`drive.file`) and *Use my existing sheet* (`spreadsheets`). | The owner's answer, 2026-08-06. It keeps the sensitive `spreadsheets` scope, which costs nothing in Testing mode and would need Google's review only if the extension were ever published to a wide audience. Dropping the second button later removes that obligation entirely. |
| 29 | **The engine installs per-user, with no administrator rights and no installer program.** A single PyInstaller `.exe` downloaded by the extension into `%LOCALAPPDATA%\ScrapeX\engine\`, plus one `HKCU\…\Uninstall\ScrapeX` key so it appears in *Apps & features*. | The owner asked for the easiest possible install with no admin. This is not a new mechanism: `scrapex/autostart.py` already puts a silent `.vbs` in the per-user Startup folder and `scrapex/nativehost.py` already registers under HKCU, commented "per-user, no admin rights needed". An MSI avoiding admin needs `ALLUSERS=""` and a per-user install — the same outcome for an extra build tool, an extra signing surface and a longer release, buying only the Add/Remove entry, which is one registry key. SmartScreen still warns once; Decision 22 accepts that. |
| 30 | **The privacy policy and the support page are checked by tests, not trusted.** | The store requires both and neither existed. Every promise that CAN be checked is: the policy must name every OAuth scope the manifest asks for and every host it may reach, must not claim "no full Drive access" while the manifest asks for it, and the shipped JavaScript must contact no analytics host. **The guard caught a real omission on its first run** — `www.googleapis.com` was reachable and undisclosed. |
| 31 | **ScrapeX's source goes private before the first release; everything a user can reach lives on the public delivery endpoint, `muhammadbayoumi/mbiX-hub`.** | GitHub answers 404 on a private repository to anyone not signed in — which is every user — so a panel reading the source repository would tell every installation the engine had never shipped, and the support link would 404 in a browser. `mbiX-hub` describes itself as the ecosystem's public delivery endpoint, already carries seventy-six of the Excel add-in's releases and already had an empty `ScrapeX/` folder waiting. `mbiXsite` was named first and was wrong: it has no releases and is not what the add-in delivers through. A test refuses any user-facing URL that names the private repository. |
| 32 | **The panel reads a version manifest at `ScrapeX/json/version.json` on `raw.githubusercontent.com` — never the GitHub releases API.** | Copied from the add-in's own update path, which has polled `Xadd-in/json/version.json` across seventy-six releases. `api.github.com` allows **sixty unauthenticated requests an hour per IP**: one owner never notices, and an office or a VPN behind a shared address starts being refused by a check nobody can fix. The manifest also carries `minimum_extension_version` and `protocol_version`, read out of the engine being shipped, so the panel can refuse an incompatible release **before** downloading it rather than after installing it. A 404 is then the honest, complete answer "nothing has been released yet" — the file is not written until the first release writes it. |
| 33 | **A release is refused unless its version is strictly newer than the one already published.** | The third thing taken from the add-in, whose own comment says why in one line: *clients would never update, or would be told to downgrade*. Both failures are silent — an equal version is published and never offered; a lower one tells every installation it is ahead of the newest engine. The check runs before the build, because the manifest is public and costs seconds while PyInstaller costs ten minutes. A `workflow_dispatch` run warns instead of failing, or a build check would stop being usable the moment a version shipped. `concurrency: release-engine` comes with it: two runs finishing minutes apart both rewrite one file, and the one that finishes **last** wins, which is not the one with the higher version. |
| 34 | **What the release workflow writes is executed against what the panel reads, in one test.** | The writer is Python inside a YAML heredoc and the reader is JavaScript in the extension. A renamed field — `installer.size` for `installer.bytes` — leaves both sides individually correct, both suites green, and the Engines page saying "the release manifest could not be read" at the one moment nothing can be un-published. The test lifts the workflow's own Python out of the YAML, runs it, and hands the output to the real reader. Proved by mutation on four field renames. |
...
- ~~Split the versions~~ — **done, then found to be half-done. See below.**
- **Unweld the two release paths.** *(done 2026-08-05, PR #112)* Two things made Decision 21 impossible and neither was visible:
  - `extension/tests/version-compat.test.mjs` asserted `manifest.version === VECTORS.version`, and `VECTORS.version` is the **engine's** number. Bumping only `extension/manifest.json` — which is exactly what a Chrome Web Store release is — failed with `actual: '0.3.0', expected: '0.2.0'`, and symmetrically on an engine-only bump. Python had already dropped that equality on 2026-08-05; the JavaScript copy never followed.
  - All seven capabilities recorded `since=VERSION`, so `MINIMUM_EXTENSION_VERSION` **was** `VERSION`. Moving the engine alone to 0.4.0 raised the floor to 0.4.0 and declared the published extension too old. Every engine release did that, to an extension that had not changed. Nothing caught it, because with `since=VERSION` the floor and the head move together and every assertion about them stays true. A capability's `since` is now the version it was introduced in, frozen; the vectors file is byte-identical afterwards.
- **Split CI by path — on a declared marker, not a directory glob.** Eleven Python test files under `tests/` assert on `extension/` sources (~154 tests), including the `panel_control` gate (`tests/test_version.py`), the dangling-id gate (`tests/test_panel_wiring.py`) and `tests/test_settings_live_in_the_extension.py`, which is the mechanical enforcement of the control-room rule. **A YAML glob routing `extension/` away from the Python job switches all of them off on precisely the PRs they police**; a glob routing it in changes nothing. Express the set as `pytest -m extension`, declared in the tests themselves, guarded by a test that fails when a file reads `extension/` without the mark, and carry `ci.yml`'s collected-count gate onto it — the panel suite once reported green for months while 48 behavioural tests silently skipped, and a split is the second way that can happen.
