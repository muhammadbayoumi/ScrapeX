...
| 31 | **ScrapeX's source goes private before the first release; everything a user can reach lives on the public site, `muhammadbayoumi/mbiXsite`.** | GitHub answers 404 on a private repository's releases endpoint to anyone not signed in — which is every user — and a 404 reads as "nothing has been released yet". Every panel would say the engine had never shipped, in a sentence honest about a fact that is wrong. The support link would 404 in a browser for the same reason. A test now refuses any user-facing URL that names the private repository. |
| 32 | **The release feed reads the LIST and picks the newest `engine-v*` tag — never `/releases/latest`.** | The public site carries several products: ScrapeX, the Excel add-in, and more. `/releases/latest` answers with whatever anyone published last, so an add-in release cut after an engine release would make the panel say "no engine has been released yet" with complete confidence and no truth in it. The tag is what says which product a release belongs to. A 404 on the LIST endpoint therefore means the site cannot be seen — renamed or private — and not that nothing is released; one sends the owner to wait and the other to look. |
