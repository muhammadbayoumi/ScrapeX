...
| [REQ-45](#req-45--the-crawl-button-does-not-work-for-muqawil) | The crawl button does not work for muqawil | **Done** — 2026-09-02, both halves on `main`: the resolver as `#301` and the collector plus the panel's own action as `#310` (`588f9047`). It was the HEAD of the muqawil queue under `R-81`, because he never uses a terminal, so every remaining muqawil step passed through a control he did not have. [R-78](RULINGS.md#r-78--the-scheduler-reads-the-registry-not-a-file-and-a-new-source-needs-no-new-code) settles the shape: **the scheduler resolves a source through `source_site`, and the button falls out of that.** `POST /api/jobs` validates against `sources.yaml`, and `scrapex/jobs.py` has zero references to any contractor module, so opening the route alone would turn a clear 404 into a job that fails mid-run (`OP-92`). **The resolver landed as `#301` and the collector is on branch `feat/the-crawl-button-drives-the-collector-it-needs`** — which is where the other half of that warning came true: the route accepted the key and the worker handed it to the PRICE collector, and every test asserted `queued`, which was true. A third job kind carries it, and **`OP-126` was the last link**: «Update now» was broken on EVERY card — `run_mode: "current"` is not a `RunMode`, so the engine answered 400 about the mode and never read the source key, since the handler was written | 2026-08-26 |
...
**Captured 2026-08-26 · root cause proven · Ruled 2026-08-30 — `R-78` settles the shape, `R-81` moves it to the HEAD of the muqawil queue · Done 2026-09-02 — the resolver as `#301`, the collector and the panel's action as `#310` (`588f9047`)**

> **CLOSED, and the chain was measured on `main` rather than on the branch that built it** — against his live warehouse, read-only: the key resolves through `source_site` (`muqawil_org`, active, `full_then_listing`); the route chooses `directory_crawl`; a worker runs that kind; the scope is honoured since `#304`; and the panel offers the action, with 31 guards green on `main`.
>
> **ONE STEP REMAINS AND IT IS NOT CODE.** `0017` widens `crawl_job`'s `job_kind` CHECK, and his warehouse reads `user_version = 16` — so it refuses the new kind until the migration reaches it. `R-64`: it may, now that it is on `main`. **The safe door is restarting the engine, not the panel's «Upgrade database» button** — `native.py`'s `_spawn_engine` runs `engine_argv("ui", ...)`, and `scrapex ui` is the path that backs up first. The button is `OP-127`.

> **THE SECOND HALF, 2026-09-02.** `#301` made this route accept `muqawil_org`; the
> worker then handed it to `capture_source`, the PRICE collector, whose result
> carries observations, duplicates, products, variants and attributes — none of
> which a contractor listing crawl produces. **So the key was accepted at the door
> and the wrong thing ran behind it**, and every test written for `#301` asserted
> `status == "queued"`, which was true. Nothing looked at the job KIND, and the kind
> is the only field that says which collector will run.
>
> A third job kind now carries it, on the route `organization_enrichment` already
> took: `directory_crawl`, with `scrapex/directoryjob.py` as its runner, reached from
> a table in `jobs.py` rather than a two-branch `if` — and `JOB_KINDS` is derived
> from that table instead of being listed a thousand lines away from it. Not one line
> of crawling was written: `contractors.crawl` stays the single implementation both
> front doors call.
>
> **What is still not his to press:** the LISTING phase is what runs. The profile
> pages are a separate collector and the job says so in its own log, so a finished
> listing crawl cannot be read as «كلّ ما ينشره الموقع».
...
| ~~OD-02~~ | ~~is `--amber` the intended colour or the accident of a clip?~~ **Ruled 2026-08-30 as `#8d5e00`, then SUPERSEDED 2026-09-02 by [R-85](RULINGS.md#r-85--the-system-is-supabases-exactly-and-supabase-is-the-only-colour-choice): it is `#ca8a10`, Supabase's own.** «القيم فقط» takes precedence over a hue-holding gamut map of a value they do not publish. **And the departure was never real** — their 2.677:1 was measured on `--amber-weak`, a pairing Supabase never renders; with their own ink it is 6.923 / 10.646, higher than what it replaced | done |
...
| ~~OD-08~~ | ~~discharge attribution under Apache-2.0 or MIT?~~ **Ruled 2026-08-30 as [R-82](RULINGS.md#r-82--the-ten-remaining-decisions-of-req-49-taken-in-one-pass): BOTH.** Closes `OP-108` | their root is Apache-2.0, `packages/ui` is MIT, and this repository already discharges the same obligation three times for Google | **both** — cheaper than deciding, and Apache-2.0 §4(b) also wants the statement of change |
| ~~OD-09~~ | ~~sanction the panel's control-height override?~~ **Ruled 2026-09-02 «احذفها» as [R-85](RULINGS.md#r-85--the-system-is-supabases-exactly-and-supabase-is-the-only-colour-choice) §5 — DELETE the floor.** It is exact match, not a preference: Supabase's control scale is 26/34/38/42/50 with no 48px floor. **And the warning that kept it parked was wrong** — the three appearance selectors measure 48/115/48 with the override and without it. The real cost was one icon button and two rendering pins | done |
