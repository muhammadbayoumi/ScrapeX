...
| [REQ-33](#req-33--the-three-dots-do-not-appear-on-a-contractor-card) | The three dots do not appear on a contractor card | **In flight** — built and proved by calling every route; merging is his | 2026-08-22 |
| [REQ-34](#req-34--the-three-dots-are-unprofessional-the-profile-pages-are-done-properly) | The three dots are unprofessional; the profile page's are done properly | **In flight** — brought to `.account-menu-button`'s treatment, both themes | 2026-08-22 |
...
## REQ-33 · The three dots do not appear on a contractor card
**Captured 2026-08-22 · In flight — built, proved route by route; merging is his**

> «ال 3 نقاط لا تظهر فى كارد مقاول»

Sent with a screenshot of the Data screen: `aramco.com` and `spark-eshop.com` each
carry a `⋮`, and `muqawil.org / contractors [Row 17,304]` and
`muqawil.org / contractor_profiles [Row 704]` carry none.

**He is asking for something this repository had already found and recorded** —
`OP-42`, written the same day from the same screenshot, one PR earlier. That is the
register boundary working as intended rather than a duplicate: we found it, he then
asked for it, and it moves here.

**It was deliberate and it was five-sixths right.** `sourceMenu` returned `""` for
`kind === "dataset"` because Update, Pause, Source settings and Export to Google
Sheets all drive routes that resolve a key through `sources.yaml`, and a dataset is
not in it. *Open the data table* was built afterwards, resolves through the dataset
catalogue, and worked the whole time it was hidden.

**What was built is not a per-entry allowlist.** A hand-written claim about the
engine's routing is what rotted the first time. Each action now declares the route
it drives and a proof of what that route does with a dataset key, and
`tests/test_a_dataset_card_offers_what_works.py` calls every one of them against a
real approved dataset — asserting both that an offered action answers with rows and
that a withheld one is genuinely unable. Measured:

| action | route | with a dataset key |
|---|---|---|
| `table` | `GET /api/table/{key}` | **200**, 4 rows, 25 columns → offered |
| `sheet` | `GET /api/export/{key}` | 404 |
| `update` | `POST /api/jobs` | 404 |
| `pause` | `POST /api/sources/{key}/active` | 404 |
| `settings` | `GET /sources/{key}` | 404 — the route exists for nobody (`OP-44`) |
| `changes` | `GET /source/{key}` | 200 with no changes section on the page |

So a contractor card carries a menu with **one live row and no greyed rows**. The
page that row opens is proved to draw a dataset too, in
`tests/test_tab_page_dom.py::test_it_draws_a_dataset_and_not_only_a_price_table` —
a route that answers and a page that renders nothing would be the same dead end one
step later.

**The hole that let this pass is closed.** `tools/panel_harness.py` carried no
dataset-kind source, so `test_dataset_action_opens_the_workspace_directly` could
assert that *every* card has a menu and stay green. It carries one permanently now,
and adding it immediately failed that assertion — 2 triggers against 3 cards. Two
further findings fell out of the same row: `OP-44` and `OP-45`.

---

## REQ-34 · The three dots are unprofessional; the profile page's are done properly
**Captured 2026-08-22 · In flight — matched to the profile page in both themes**

> «توجد ال3 نقاط بشكل غير احترافى فوق الكارت وداخل مربع اعتقد ان ال3 نقاط معمولة فى
> صفحة profile بشكل احترافى عن هذا الشكل»

Two claims, and **both are correct**. He named the reference himself, which is what
made this measurable instead of a matter of taste. Read off the two screens before
anything was changed:

| | the Data card | the Profile row |
|---|---|---|
| size | **48x48** | 40x40 |
| `border-radius` | **`0 8px 8px 0`** | `999px` |
| background | **filled** (`--surface`, `--accent-weak` while open) | transparent |
| border | **1px `--line-strong`** | transparent |
| shadow | **`--shadow-xs`** | none |
| colour | `--text` | `--muted` |

**`0 8px 8px 0` is the entire diagnosis.** `.split-button-trigger` rounds its OUTER
corners only, because in a split button its inner edge butts against the primary
action it sits beside. A card has no primary action, so the shared rule drew a
lopsided box with two square corners, sitting on the card's own rounded corner —
"inside a box, above the card", which is exactly what he wrote.

**Fixed by bringing it to `.account-menu-button`'s treatment**: a bare 40px circle,
`--muted`, tinted with `--control-hover` on hover and while open. Every value is a
token the profile row already uses, so nothing needed a new palette colour — which
also means it cannot ignore his palette (a new token has four homes; see
[LESSONS.md](LESSONS.md)). The rules are local to `.dataset-card`:
`components.css` is generated from `design/components.css`, and the Activity log's
split button — which does have a primary half and is not broken — must not move.

**Guarded by comparison, not by numbers.** `test_panel_dom.py::
test_the_card_menu_trigger_is_dressed_like_the_profile_pages` reads both controls'
computed styles in the same browser and asserts they agree, parametrised over light
and dark. Hard-coded values would pass the day the profile page changes and the card
does not, which is the drift that produced two different three-dots buttons in the
first place.
