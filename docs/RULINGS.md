...
so nothing can recompute them. **A merge cannot carry price history at all — and
nothing has asked it to yet**, which is the narrower and more useful claim: the
route is missing, not the rows. That is `R-32`'s correction — price is one category, not the
