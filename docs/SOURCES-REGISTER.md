...
**16 sites, all sent by the owner on 2026-07-31 except the last, none probed.** No request
...
*"maybe prices not included"* caveat — see **Q-B**: a market site with no prices is not a
price source but an **enrichment-only** one, and that family (`datasheet-enrichment`) is
declared with **no builder** and is already wanted by **DEC-5**.
