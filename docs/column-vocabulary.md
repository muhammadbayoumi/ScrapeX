...
while the export ALSO had a `country`, and `curation` was "Curation".
...
`price_*` family, `tax -> tax`, `curation -> curation`, and
`open`/`product_link -> product_link`. Two of those need a decision before
...
- **`open` and `product_link` both target `product_link`**, which is an
...
| `price` (headed "Price") | `price` | Price |
| `price_before` | `price_before` | Price before |
| `price_sale` | `price_sale` | Sale price |
| `price_usd` | `price_usd` | Price (USD est.) |
| `price_previous` | `price_previous` | Previous price |
| `price_change` | `price_change` | Price change |
| `price_min` / `price_max` | `price_min` / `price_max` | Lowest price / Highest price |
| `discount` (one text cell) | `discount` + `discount_pct` | Discount · Discount % |
| `unit` | `unit` | Unit |
| `currency` | `currency` | Currency |
| `availability` (headed "Status") | `availability` | Availability |
| `tax` (headed "Tax") | `tax` | Tax |
| `tax_included` | `tax_included` | Tax included |
| `tax_evidence` | `tax_evidence` | Tax evidence |
| `tax_rate_pct` | `tax_rate_pct` | Tax rate % |
| `tax_statement` | `tax_statement` | Tax statement |
...
| `official_source_link` | `official_source_link` | Official source link |
| `curation` (headed "Curation") | `curation` | Curation |
| `open` (blank heading) | `product_link` | *(icon only)* |
| `product_link` (export only) | `product_link` | Product link |
