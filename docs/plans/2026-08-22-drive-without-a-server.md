...
drive» ([REQ-32](../REQUESTS.md#req-32--open-a-session-that-starts-on-the-drive-plan),
...
# READ THIS FIRST · `merge-warehouse` silently discards the other machine's entire price history

**This is not a phase-ordering detail and it is not a future risk. It is true of the
command he was told to run, today.** `OP-46` in [BACKLOG.md](../BACKLOG.md) carries
the full working; this section exists because it must not be found by reading down
to §3.4.
...
**It cannot be determined from this side, and that is itself the finding.**
...
- **And it does not change the answer.** Whether or not `--from` was supplied, no
  price row could have crossed, because there is no INSERT for those tables. Any
  merge that ran dropped **100%** of the other machine's price history and left no
  trace that it had.

**What would settle it, and it is two numbers from the other machine:**
`scrapex merge-warehouse --status`, and `SELECT COUNT(*) FROM price_observation`.
Nothing else is needed and nothing on this machine can substitute for them.
...
| `scrapex/webui/app.py:2589` `POST /api/bundle` | the engine's side; hands the panel a name, a size and a digest |
...
`scrapex/cli.py:164` `_cmd_merge_warehouse`. It is live **on his own warehouse
...
**A merge today therefore silently discards the other machine's entire price
history.** It is not wrong about what it does; its docstring says "only the
...
(`REQ-32`) · `docs/BACKLOG.md` (**`OP-46`** — the price merge — plus `OP-44`,
`OP-45`, `Q-19`–`Q-23`) ·
