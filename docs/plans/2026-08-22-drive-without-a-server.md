...
`OP-50` is the work that gives it one.
...
defect in the code, the second is why nothing is lost *so far*. `OP-50` in
...
### And the risk is MISATTRIBUTION, not duplication — measured

A merge could never *double* an observation. `ux_price_obs_dedupe` is UNIQUE on
`(offer_id, business_date, record_hash)`, `record_hash` is content-derived, and it
is **NULL on 0 of 92,740 rows** with **92,740 distinct triples over 92,740 rows**.
Re-running a fixed merge is therefore idempotent by construction.

**The hazard is the opposite one, and the unique index would wave it through.**
`offer_id` is machine-local. A copy-verbatim insert of the other machine's rows
would hang its price history on *whichever* offers happen to carry those ids here —
unrelated products, silently, with every triple still distinct and the constraint
still satisfied. That is why §3's fix is a remap and not an insert, and why
"proven by count" is not a sufficient gate on its own: the count would be right.

**The remap chain resolves, and one link needed checking rather than assuming.**
`source_key → external_product_id → external_variant_id | option_fingerprint →
country + segment + branch`. It was put to me that `source_product` *"declares no
unique index at all"*, which would have made the first hop hope rather than proof
and cost a migration to fix. **It is not so, and the way the claim arose is the
lesson.** `source_product` carries an inline table constraint —

    UNIQUE (source_id, external_product_id)  -- the owner's mandated constraint

— which SQLite implements as `sqlite_autoindex_source_product_1`
(`unique=1`, `origin='u'`). It is enforced today, and the data satisfies it:
**9,270 rows, 9,270 distinct pairs, 0 NULL `external_product_id`**. Nothing needs
declaring and no migration is owed.

**Why two of us missed it:** an auto-index created by a `UNIQUE` constraint has
`sql = NULL` in `sqlite_master`, so the natural query —
`SELECT sql FROM sqlite_master WHERE type='index' AND sql IS NOT NULL` — cannot see
it. `PRAGMA index_list` can. This plan's own §3.4 table was built with the first
query and the same blind spot, which is [LESSONS §9](../LESSONS.md) — *a measurement
is only as good as the instrument* — arriving through a new door.
...
Filed as [`OP-45`](../BACKLOG.md) and [`OP-49`](../BACKLOG.md); the fix is Phase 1
...
1. **`merge-warehouse --force`, and delete `--claim`.** `OP-45`, `OP-49`.
...
3. **THE PRICE MERGE — and it comes FIRST within this phase, ahead of items 1 and
   2.** The whole working is at the top of this file and in `OP-50`; it is not
...
(`REQ-34`) · `docs/BACKLOG.md` (**`OP-50`** — the price merge — plus `OP-45`,
`OP-49`, `Q-19`–`Q-23`) ·
...
**Nothing in Phase 0 changes `warehousemerge.merge` itself.** `OP-50` is filed and
