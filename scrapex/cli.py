...
def _render_agents() -> str:
    """`AGENTS.md`, carrying `CLAUDE.md` whole.

    AN AGENT THAT IS NOT CLAUDE LOADS THIS NAME, not `CLAUDE.md`, and `main` has no
    branch protection -- GitHub offers Pro or a public repository and this one is
    neither -- so nothing mechanical stops such an agent pushing. The rules document
    is the only control there is, which is why this file carries the rules rather
    than a pointer to them: a pointer is an instruction an agent may skip, and there
    is no second line of defence behind it.

    GENERATED, so there is one source of truth and it stays `CLAUDE.md`. The first
    copy written under this name was written by another agent that found no such
    file, and it had already mangled a path by the time anyone read it.
    """
    header = """<!-- GENERATED from CLAUDE.md by `python -m scrapex.cli export-version`.
     Do not edit this file. Edit CLAUDE.md; this is a copy of it. -->

# ScrapeX — the rules, for any agent

You are reading a generated copy of `CLAUDE.md`. It is here because tools other than
Claude Code look for this name. Read all of it before you change anything, and make
your edits to `CLAUDE.md`, never to this file.

---

"""
    return header + (ROOT_DIR / "CLAUDE.md").read_text(encoding="utf-8")
...
def _cmd_export_version(args: argparse.Namespace) -> int:
    ...
    agents.write_text(_render_agents(), encoding="utf-8")
    print(f"wrote {agents} (CLAUDE.md, for the tools that read this name)")
    return 0
...
def _cmd_peek(args: argparse.Namespace) -> int:
    ...
    if sample:
        ...
        for row in sample:
            vat = "incl" if row["tax_included"] else "excl"
            name = (row["name"] or "")[:48]
