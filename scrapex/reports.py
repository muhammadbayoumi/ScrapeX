...
_LATEST_PER_OFFER = (
    ...
    "  WHERE po2.offer_id = po.offer_id ORDER BY po2.observed_at DESC LIMIT 1)"
)
...
def _browse_filters(search: str | None, availability: str | None) -> tuple[str, list]:
    clause, params = "", []
    if search:
        clause += " AND sp.source_name LIKE ?"
...
def browse_observations(conn: sqlite3.Connection, source_key: str, *, search: str | None = None,
                        availability: str | None = None, offset: int = 0, limit: int = 50) -> BrowsePage:
    ...
    rows = conn.execute(
        ...
        "       po.business_date, sp.product_url, sp.curation_status "
        f"{_LATEST_PER_OFFER}{filt} ORDER BY sp.source_name LIMIT ? OFFSET ?",
...
EXPORT_HEADER = [
    "product_name", "option_label", "sku", "effective_price", "regular_price",
    "sale_price", "currency", "availability", "vat_included", "business_date", "product_url",
]
...
def export_source_table(conn: sqlite3.Connection, source_key: str,
    ...
    rows = conn.execute(
        ...
        "       po.vat_included, po.business_date, sp.product_url "
        f"{_LATEST_PER_OFFER} ORDER BY sp.source_name LIMIT ?",
        (source_key, limit),
    ).fetchall()
    table = [
...
def recent_observations(conn: sqlite3.Connection, source_key: str, limit: int = 10) -> list[dict]:
    ...
    rows = conn.execute(
        ...
        "       po.vat_included, po.business_date "
        "FROM price_observation po "
        "JOIN source_offer so ON so.offer_id = po.offer_id "
        "JOIN source_variant sv ON sv.source_variant_id = so.source_variant_id "
        "JOIN source_product sp ON sp.source_product_id = sv.source_product_id "
        "JOIN source_site ss ON ss.source_id = sp.source_id "
        "WHERE ss.source_key = ? ORDER BY po.observed_at DESC LIMIT ?",
        (source_key, limit),
    ).fetchall()
    return [
