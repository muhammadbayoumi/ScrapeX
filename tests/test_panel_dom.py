...
def _asked(page) -> int:
    ...
    some PASS when they run together: a sentence on the screen is not evidence
    that this test's own stub put it there. The counter is zeroed by this test's
    stub, so a non-zero value cannot have come from anywhere else -- which makes
    each test independent of what ran before it.
    """
