...
WORSE, AND THIS IS THE PART WORTH KEEPING. Every one of the 78 listing-served snapshots
on disk is caught by layer 1, so on today's corpus layer 2 is unreachable: the candidate
raises before the cross-check is consulted. The reviewer proved `mismatched` cannot fire
on any stored page. That does not make layer 2 pointless — it is the guard for a page of
the RIGHT shape carrying the WRONG contractor, which layer 1 only catches when the caller
passes an id — but it does mean the only way to see it work is a test that constructs the
case. This file is that test.
