...
disk on 2026-08-20 with nothing re-fetched.
  ...
  python tools/crawl_muqawil_listing.py --plan            # 56 sizing requests
  python tools/crawl_muqawil_listing.py --crawl --run-ref listing-2026-08-20
  python tools/crawl_muqawil_listing.py --approve --run-ref listing-2026-08-20
...
# trap: `pip install -e` points at the MAIN checkout, so a tool run from a
# worktree imports main's code and its own tests stay green about nothing. Put
# this file's own tree first and say which one won.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

# UTF-8 ON A cp1252 CONSOLE. See `say`: the sizing pass once finished every request
# and then died printing its summary. Reconfigured where possible, guarded in `say`
# where it is not — an older Windows stream may refuse this too.
...
LOG = Path.home() / ".scrapex" / "trial" / "muqawil-listing.log"
